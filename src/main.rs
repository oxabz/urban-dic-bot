...
use serenity::{
    async_trait,
    model::{gateway::Ready, interactions::Interaction, guild::Guild},
    prelude::*,
};
...
impl EventHandler for Handler {
    ...
    async fn guild_create(&self, ctx: Context, guild: Guild, is_new: bool) {
        ...
        guild.id.set_application_commands(&ctx.http, |cmd|{
            cmd.create_application_command(DefinitionCommand::create)
        }).await.expect("Error setting application commands");
    }
    ...
    async fn interaction_create(&self, ctx: Context, msg: Interaction) {
        match msg{
            ...
            Interaction::MessageComponent(msg)=>{
                match msg{
