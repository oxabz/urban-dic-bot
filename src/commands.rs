...
impl DefinitionCommand {
    ...
    pub async fn handle(ctx: &Context, msg: &ApplicationCommandInteraction) -> bool {
        ...
        msg.create_interaction_response(ctx, move |resp| {
            resp.interaction_response_data(move |data| {
                ...
                data.components(|comp| {
                    Self::page_componnent(comp, interaction_id, word.as_str(), 0, defs.len())
                });
                data
            });
        ...
        .await
        .expect("DefinitionCommand : Error creating response");

        ctx.data
            .write()
            .await
            .get_mut::<DefinitionInteractionData>()
            .unwrap()
            .insert(interaction_id, msg.clone());
    ...
    pub async fn handle_msg(ctx: &Context, msg: &MessageComponentInteraction) -> bool {
        match msg.data.custom_id.as_str() {
            id if id.starts_with("page#") => {
                ...
                let original_interaction = lock
                    .get::<DefinitionInteractionData>()
                    .unwrap()
                    .get(&interaction_id)
                    .unwrap();
                ...
                original_interaction
                    .edit_original_interaction_response(ctx, |resp| {
                        ...
                        resp.components(|e| {
                            Self::page_componnent(e, interaction_id, word, page, defs.len())
                        });
                        resp
                    })
                    .await
                    .expect("DefinitionCommand : Error editing response");

                msg.create_interaction_response(ctx, |res| {
                    res.kind(serenity::model::interactions::InteractionResponseType::UpdateMessage)
                })
                .await
                .expect("DefinitionCommand : Error creating response");
            }
    ...
    fn page_componnent<'a>(
        comp: &'a mut CreateComponents,
        interaction_id: u64,
        word: &str,
        page: usize,
        total_pages: usize,
